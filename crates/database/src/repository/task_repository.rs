...
impl TaskRepository {
    ...
    pub async fn create(
        ...
        let _result = task::Entity::insert(task).exec(db).await?;
    ...
    /// 根据ID查找任务
    pub async fn find_by_id(db: &DatabaseConnection, task_id: Uuid) -> Result<Option<task::Model>> {
        task::Entity::find_by_id(task_id)
            .one(db)
            .await
    ...
    /// 根据项目ID查找任务
    pub async fn find_by_project(db: &DatabaseConnection, project_id: Uuid) -> Result<Vec<task::Model>> {
        task::Entity::find()
    ...
    /// 根据父任务ID查找子任务
    pub async fn find_subtasks(db: &DatabaseConnection, parent_task_id: Uuid) -> Result<Vec<task::Model>> {
        task::Entity::find()
    ...
    /// 根据状态查找任务
    pub async fn find_by_status(db: &DatabaseConnection, project_id: Uuid, status: &str) -> Result<Vec<task::Model>> {
        task::Entity::find()
            .filter(task::Column::ProjectId.eq(project_id))
    ...
    /// 根据分配的Agent查找任务
    pub async fn find_by_agent(db: &DatabaseConnection, agent_id: Uuid) -> Result<Vec<task::Model>> {
        task::Entity::find()
    ...
    /// 查找顶级任务（没有父任务的任务）
    pub async fn find_top_level_tasks(db: &DatabaseConnection, project_id: Uuid) -> Result<Vec<task::Model>> {
        task::Entity::find()
            .filter(task::Column::ProjectId.eq(project_id))
    ...
    /// 删除任务
    pub async fn delete(db: &DatabaseConnection, task_id: Uuid) -> Result<()> {
